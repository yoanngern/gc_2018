jQuery(document).ready(function($){var item_path=new Object;var all_exceptiondata=[];var xid=-1;$('ul.categorychecklist ul.children li[style="display:none"]').parent().prevAll('input.menu-item-checkbox').next('span').html(' + ');$('input.menu-item-checkbox').nextAll('span').click(function(e){$(this).parent().children('ul.children').children('li').toggle();if($(this).nextAll('ul.children').length){if($(this).html()==' + '){$(this).html(' &ndash; ');}else{$(this).html(' + ');}}
e.preventDefault();});$(document).on('click','ul.categorychecklist li label',function(e){$(this).prevAll('input.menu-item-checkbox').trigger('click');});$('.add-to-menu .waiting').hide();$("#pp_save_exceptions input.button-primary").click(function(){$('input[name="member_csv"]').val($("input#member_csv").val());$('input[name="group_name"]').val($("input#group_name").val());$('input[name="description"]').val($("input#description").val());$("#pp_new_x_submission_msg").html(ppRestrict.submissionMsg);$("#pp_new_x_submission_msg").show();});$('#agent-profile #submit').click(function(e){$('#pp_review_exceptions').hide();$('#pp_add_exception').remove();});$(document).on('click',"#pp_tbl_exception_selections .pp_clear",function(e){var xid=$(this).closest('tr').find('input[name="pp_xid[]"]').val();if(typeof all_exceptiondata[xid]!='undefined'){delete all_exceptiondata[xid];}
$(this).closest('tr').remove();e.stopPropagation();});$('.pp_clear_all').click(function(){$('.pp_clear').trigger('click');});$(".menu-item-checkbox").click(function(){item_checkbox_click('menu-item',$(this));});$(".pp_group-item-checkbox").click(function(){item_checkbox_click('pp_group-item',$(this));});$(".pp_net_group-item-checkbox").click(function(){item_checkbox_click('pp_net_group-item',$(this));});var item_checkbox_click=function(data_var,t){var expr=data_var+'\\[(\[^\\]\]*)';var re=new RegExp(expr);itemdata=t.closest('li').getItemData();if(t.closest('div.tabs-panel').parent().hasClass('hierarchical')){x_ajax_ui('get_item_path',x_update_item_path,itemdata['menu-item-object-id']);}}
var timer;var x_update_item_path=function(data,txtStatus){var item_info=data.split('\r');item_path[item_info[0]]=item_info[1];$('input.menu-item-checkbox[value="'+item_info[0]+'"]').nextAll('label').attr('title',item_info[1]);clearTimeout(timer);}
$(document).on('mouseenter','div.hierarchical ul.categorychecklist li label',function(){if($(this).attr('title')==undefined||$(this).attr('title')==''){var that=this;timer=setTimeout(function(){itemdata=$(that).closest('li').getItemData();x_ajax_ui('get_item_path',x_update_item_path,itemdata['menu-item-object-id']);},500);}});$(document).on('mouseleave','ul.categorychecklist li label',function(){clearTimeout(timer);});$(document).on('click','.submit-add-item-exception',function(e){x_add_item_exception('menu-item');return false;});$(document).on('click','.submit-add-pp_group-exception',function(e){x_add_item_exception('pp_group-item');return false;});$(document).on('click','.submit-add-pp_net_group-exception',function(e){x_add_item_exception('pp_net_group-item');return false;});var x_add_item_exception=function(data_var){$('div.pp-ext-promo').hide();var items=$('#menu-settings-column').find('.tabs-panel-active .categorychecklist li input:checked');if(!$('select[name="pp_select_x_operation"]').val()){$('#pp_item_selection_msg').html(ppRestrict.noOp);$('#pp_item_selection_msg').addClass('pp-error-note');$('#pp_item_selection_msg').show();return false;}
...
$('.pp-save-exceptions').show();$(items).each(function(item_index){var t=$(this);var expr=data_var+'\\[(\[^\\]\]*)';var re=new RegExp(expr);itemdata=t.closest('li').getItemData();if(typeof(itemdata['menu-item-object-id']!='undefined')){item_caption=itemdata['menu-item-title'];if(hier_type){if(typeof(item_path[itemdata['menu-item-object-id']])!='undefined')
...
var reload_mod_type=function(){if($('select[name="pp_select_x_operation"]').val()){setTimeout(function(){x_ajax_ui('get_mod_options',draw_mod_types)},100);}else
$('.pp-select-x-mod-type').hide();}
var reload_assign_for=function(){if($('select[name="pp_select_x_for_type"]').val())
setTimeout(function(){x_ajax_ui('get_assign_for_ui',draw_assign_for)},100);else
$('.pp-select-x-assign-for').hide();}
var reload_status=function(){var op=$('select[name="pp_select_x_operation"]').val();var mod_type=$('select[name="pp_select_x_mod_type"]').val();if(mod_type&&op){setTimeout(function(){x_ajax_ui('get_status_ui',draw_status)},50);if('include'==mod_type){$('input.add-to-top').show();$('input.add-to-top').parent().show();}else{$('input.add-to-top').hide();$('input.add-to-top').parent().hide();}}else
$('.pp-select-x-status').hide();if('include'==mod_type||(('exclude'==mod_type)&&('associate'==op))){$('td.pp-select-items input.menu-item-checkbox[value="0"]').closest('li').show();}else{$('td.pp-select-items input.menu-item-checkbox[value="0"]').closest('li').hide();}}
$('select[name="pp_select_x_for_type"]').bind('change',reload_operation);$('select[name="pp_select_x_for_type"]').change(function(){$('.pp-select-items').hide();$('.pp-select-x-mod-type').hide();$('.pp-select-x-via-type').hide();$('.pp-select-x-status').hide();$('#pp_add_exception').css('width','auto');});$('select[name="pp_select_x_operation"]').bind('change',reload_via_type);$('select[name="pp_select_x_operation"]').bind('change',reload_mod_type);$('select[name="pp_select_x_operation"]').bind('change',reload_status);$('select[name="pp_select_x_mod_type"]').bind('change',reload_status);$('select[name="pp_select_x_via_type"]').bind('change',reload_status);$('select[name="pp_select_x_via_type"]').bind('change',reload_assign_for);$('select[name="pp_select_x_via_type"]').change(function(){$('#pp_add_exception .postbox').hide();if($(this).val()){var pp_via_type=$(this).val();$('#select-exception-'+pp_via_type).show();$('.pp-select-items').show();}else
$('.pp-select-items').hide();$('#pp_add_exception').css('width','100%');$('input.menu-item-checkbox').prop('checked',false);});var draw_operations=function(data,txtStatus){sel=$('select[name="pp_select_x_operation"]');sel.html(data);sel.triggerHandler('change');$('.pp-select-x-operation').show();x_ajax_ui_done();}
